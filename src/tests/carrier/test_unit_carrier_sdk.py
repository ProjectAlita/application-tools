...
class TestCarrierSDK:
    ...
    @pytest.fixture
    def client(self, credentials, mock_session):
        with patch('alita_tools.carrier.carrier_sdk.requests.Session', return_value=mock_session):
            client = CarrierClient(credentials=credentials)
            client.session = mock_session
            return client
    
    def test_initialization(self, credentials):
        with patch('alita_tools.carrier.carrier_sdk.requests.Session') as mock_session_class:
    ...
    def test_request_success(self, client, mock_session):
        ...
        mock_session.request.assert_called_once_with("GET", "https://carrier.example.com/api/endpoint")
        assert result == {"success": True, "data": [1, 2, 3]}
    
    def test_request_http_error(self, client, mock_session):
        mock_response = MagicMock()
    ...
    def test_request_json_decode_error(self, client, mock_session):
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
    ...
    @patch('os.remove')
    @patch('shutil.rmtree')
    def test_download_and_unzip_reports(self, mock_rmtree, mock_remove, mock_zipfile, client, mock_session):
        mock_response = MagicMock()
    ...
    def test_get_report_file_name(self, client):
        with patch.object(client, 'request') as mock_request, \
             patch.object(client, 'download_and_unzip_reports') as mock_download:
            
            # Mock the report info request
            mock_request.side_effect = [
                {"name": "Test Report", "build_id": "build-123"},  # First call for report info
                {"rows": [{"name": "reports_test_results_build-123.zip"}]}  # Second call for bucket files
            ]
    ...
    def test_upload_excel_report(self, mock_post, client):
        with patch("builtins.open", mock_open()) as mock_file:
            ...
            # Check that the URL and headers are correct
