...
class TestCarrierApiWrapper:
    ...
    def test_init_success(self, mock_init_validator, wrapper_config, mock_carrier_client):
        ...
        expected_credentials = CarrierCredentials(
            url=wrapper_config["url"],
            token=wrapper_config["private_token"], # Pass the SecretStr directly
            organization=wrapper_config["organization"],
            project_id=wrapper_config["project_id"]
        )
        ...
        assert call_kwargs['credentials'].url == expected_credentials.url
        # Compare the unwrapped secret string
        assert call_kwargs['credentials'].token.get_secret_value() == expected_credentials.token.get_secret_value()
        assert call_kwargs['credentials'].organization == expected_credentials.organization
        assert call_kwargs['credentials'].project_id == expected_credentials.project_id
    ...
    def test_init_missing_project_id(self, wrapper_config):
        ...
        # Instantiation happens *inside* the context manager
        with pytest.raises(ValueError, match="project_id is required and cannot be empty."):
    ...
    def test_init_client_exception(self, mock_sdk_client, wrapper_config):
        ...
        # Instantiation happens *inside* the context manager, triggering the patched validator
        # The validator catches the exception and re-raises it.
        with pytest.raises(Exception, match="Initialization failed: SDK Init Failed"):
    ...
    # This test was duplicated, remove the duplicate and keep the original unpatched one below.
    # @pytest.mark.positive
    # @patch('src.alita_tools.carrier.api_wrapper.CarrierAPIWrapper.initialize_client', return_value=None)
    # def test_fetch_test_data(self, mock_init_validator, wrapper_config, mock_carrier_client):
    #     """Test fetch_test_data calls the client method."""
    ...
    # Remove the duplicated test entirely
    # @pytest.mark.positive
    # @patch('src.alita_tools.carrier.api_wrapper.CarrierAPIWrapper.initialize_client', return_value=None)
    # def test_fetch_test_data(self, mock_init_validator, wrapper_config, mock_carrier_client):
    #     """Test fetch_test_data calls the client method."""
