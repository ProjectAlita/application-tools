...
class TestConfluenceAPIWrapper:
    ...
    @pytest.mark.skip(reason="ModuleNotFoundError: No module named 'alita_sdk'")
    @pytest.mark.positive
    def test_index_pages(self, api_wrapper):
        from langchain_core.documents import Document
        with patch.object(ConfluenceAPIWrapper, '_loader', return_value=iter([Document(page_content="doc", metadata={})])) as _loader:
