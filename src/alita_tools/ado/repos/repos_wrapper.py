...
from azure.devops.v7_0.git.models import GitVersionDescriptor, GitPullRequestSearchCriteria, \
    GitPullRequest, GitPush, GitCommit, \
    Comment, GitPullRequestCommentThread, \
    GitCommitRef, GitCommitRef, GitRefUpdate
...
class RepoConfig(BaseModel):
    ...
    repository_id: str = Field(default_factory=lambda: os.getenv("AZURE_DEVOPS_REPOSITORY_ID"))
    token: str = Field(default_factory=lambda: os.getenv("AZURE_DEVOPS_TOKEN"))
    base_branch: str = Field(default_factory=lambda: os.getenv("AZURE_DEVOPS_BASE_BRANCH", "master"))
...
class ReposApiWrapper(GitClient):
    ...
    def set_active_branch(self, branch_name: str) -> str:
        ...
        """
        current_branches = [
            branch.name for branch in self.get_branches(
    ...
    def _get_files(self, directory_path: str = "", branch_name: str = "master", recursion_level: str = "Full") -> str:
        """
        Params:
        ...
        try:
            ...
            return f"Failed to fetch files from directory due to an error: {str(e)}"
        files = []
        while items:
            item = items.pop(0)
    ...
    def get_files_from_directory(self, directory_path: str) -> str:
        ...
        """
        return self._get_files(directory_path=directory_path, branch_name=self.active_branch)

    def get_pull_request_comment_threads(self, project: str, repository_id: str, pull_request_id: int) -> List[GitPullRequestCommentThread]:
        """
    ...
    def parse_pull_request_comments(self, comment_threads: List[GitPullRequestCommentThread]) -> List[dict]:
        """
        ...
        for thread in comment_threads:
            for comment in thread.comments:
                parsed_comments.append({
                    ...
                    "published_date": comment.published_date.strftime("%Y-%m-%d %H:%M:%S %Z") if comment.published_date else None,
    ...
    def list_open_pull_requests(self) -> str:
        ...
        pull_requests = self.get_pull_requests(
            repository_id=self.repository_id,
            project=self.project,
            search_criteria=GitPullRequestSearchCriteria(repository_id=self.repository_id, status="active")
            )

        if pull_requests:
            parsed_prs = self.parse_pull_requests(pull_requests)
            parsed_prs_str = (
                "Found " + str(len(parsed_prs)) + " open pull requests:\n" + str(parsed_prs)
            )
    ...
    def get_pull_request(self, pull_request_id: str) -> str:
        ...
        try:
            ...
            raise Exception(f"Failed to find pull request with '{pull_request_id}' ID. Error: {e}")
    ...
    def parse_pull_requests(self, pull_requests: Union[GitPullRequest, List[GitPullRequest]]) -> List[dict]:
        """
    ...
    def generate_diff(self, base_text, target_text, file_path):
        ...
        diff = difflib.unified_diff(base_lines, target_lines, fromfile=f"a/{file_path}", tofile=f"b/{file_path}")
        ...
        for i in range(1000):
            try:
                ref_update = {
                    ...
                    "newObjectId": base_branch.commit.commit_id
                }
                ...
                if "Reference already exists" in str(e):
                    ...
                    raise Exception(f"Failed to create branch. Error: {e}")
        return (
        ...
        try:
            ...
            try:
                self.get_item(
                    ...
                    project=self.project,
                    path=file_path,
                    version_descriptor= GitVersionDescriptor(version=self.active_branch, version_type="branch")
                )
                return (
            ...
            if branch is None or not hasattr(branch, "commit") or not hasattr(branch.commit, "commit_id"):
                return f"Branch `{self.active_branch}` does not exist or has no commits."
            ...
            ref_update = GitRefUpdate(name=f"refs/heads/{self.active_branch}", old_object_id=latest_commit_id)
            new_commit = GitCommit(comment=f"Create {file_path}", changes=[change])
            push = GitPush(commits=[new_commit], ref_updates=[ref_update])
            self.create_push(push=push, repository_id=self.repository_id, project=self.project)
    ...
    def delete_file(self, branch_name: str, file_path: str) -> str:
        ...
        try:
            ...
            branch = self.get_branch(repository_id=self.repository_id, project=self.project, name=branch_name)
            if not branch:
            ...
            ref_update = GitRefUpdate(name="refs/heads/" + branch_name, old_object_id=current_commit_id, new_object_id=None)
            push = GitPush(commits=[new_commit], ref_updates=[ref_update])
            self.create_push(push=push, repository_id=self.repository_id, project=self.project)
    ...
    def comment_on_pull_request(self, comment_query: str):
        ...
        try:
            ...
            comment_thread = GitPullRequestCommentThread(comments=[comment], status="active")
            self.create_thread(comment_thread, repository_id=self.repository_id, pull_request_id=pull_request_id, project=self.project)
